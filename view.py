...
def user_loader(usr_id):
    return db.get_or_404(User, usr_id)


@login_manager.unauthorized_handler
def unauthorized_handler():
    flash('Unauthorization first!', 'danger')
...
def is_authorized():
    ...
    ret = {'auth': True if current_user.is_authenticated else False}
    if ret['auth']: ret['admin'] = True if current_user.id==1 else False
...
class Comment(db.Model):
    ...
    body: Mapped[int] = mapped_column('comment', String)
...
def register():
    ...
    if request.method == "POST":
        ...
        password = reg_form.passwrod.data
        
        if db.session.execute(select(User).filter(User.email == email)).scalar_one_or_none():
            flash("e-Mail already exited!", 'warning')
        else:
            db.session.add(User(
                name = name,
                email = email,
                password = generate_password_hash(password=password, method='pbkdf2', salt_length=25)
            ))
            db.session.commit()
...
def login():
    ...
    if request.method == "POST":
        ...
        if user := db.session.execute(select(User).filter(User.email==email)).scalar_one_or_none():
            ...
            flash(f"User did not exist!", "warning")
...
def show_post(post_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash("Comment submitted successfuly.", 'success')
...
def delete_comment(id, pid):
    ...
    db.session.delete(db.get_or_404(Comment, id))
    print("lets commit")
    db.session.commit()
